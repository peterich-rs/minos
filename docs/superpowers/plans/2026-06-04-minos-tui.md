...
**Architecture:** Layered TUI with an `AgentBackend` trait (default `EmbeddedBackend` wrapping `AgentManager`), per-session `AgentTranslationState` translating `RawIngest` → `UiEventMessage`, and ratatui rendering with four panels (status bar, session list, chat, input bar). Three async pumps (ingest, manager events, terminal) feed a unified `mpsc` channel consumed by the event loop.
...
| `crates/minos-tui/src/ui/thread_list.rs` | Session list component |
...
impl AgentBackend for EmbeddedBackend {
    ...
    async fn send_message(&self, session_id: &str, text: &str) -> Result<()> {
        self.manager
            .send_user_message(session_id, text.to_owned())
            .await
            .map_err(Into::into)
    }
...
impl AgentTranslationState {
    pub fn new(agent: AgentName, session_id: String) -> Self {
        match agent {
            AgentName::Codex => Self::Codex(CodexTranslatorState::new(session_id)),
            AgentName::Claude => Self::Claude(ClaudeTranslatorState::new(session_id)),
            AgentName::Gemini => Self::Gemini(GeminiTranslatorState::new(session_id)),
            AgentName::Opencode => Self::Opencode(OpencodeTranslatorState::new(session_id)),
        }
    }
...
### Task 8: Implement session list component

**Files:**
- Create: `crates/minos-tui/src/ui/thread_list.rs`

- [ ] **Step 1: Write session list widget**
...
pub fn render_thread_list(
    ...
    let items: Vec<ListItem> = sessions
        ...
        .map(|(i, snap)| {
            ...
            let tid_short = &snap.session_id[..8.min(snap.session_id.len())];
            ...
            let line = ratatui::text::Line::from(vec![
                ...
                ratatui::text::Span::styled(
                    format!("{:?} ", agent_kind_short(&snap.workspace, &snap.session_id)),
                    state_style,
                ),
                ratatui::text::Span::styled(
...
fn agent_kind_short(_workspace: &std::path::PathBuf, _session_id: &str) -> &'static str {
    "agent"
}
```
...
git commit -m "feat(tui): add session list component"
```
...
impl<B: AgentBackend + 'static> App<B> {
    ...
    pub async fn handle_event(&mut self, event: AppEvent) {
        match event {
            AppEvent::Ingest(ingest) => {
                let tid = ingest.session_id.clone();
    ...
    async fn handle_manager_event(&mut self, event: ManagerEvent) {
        match event {
            ManagerEvent::SessionAdded {
                ...
                self.ui.sessions.push(ThreadEntry {
                    session_id,
                    workspace,
                    agent,
            ...
            ManagerEvent::SessionClosed { session_id, .. } => {
                if let Some(entry) = self
                    .ui
                    .sessions
                    .iter_mut()
            ...
            ManagerEvent::InstanceCrashed {
                ...
                for tid in affected_threads {
                    if let Some(entry) = self.ui.sessions.iter_mut().find(|t| t.session_id == tid) {
                        entry.state = minos_agent_runtime::ThreadState::Closed {
    ...
    async fn handle_key(
        ...
        if key.modifiers.contains(KeyModifiers::CONTROL) {
            match key.code {
                ...
                KeyCode::Char('c') => {
                    if let Some(tid) = self.ui.current_session_id().map(String::from) {
                        if let Err(e) = self.backend.interrupt_session(&tid).await {
                ...
                KeyCode::Char('d') => {
                    if let Some(tid) = self.ui.current_session_id().map(String::from) {
                        if let Err(e) = self.backend.close_session(&tid).await {
    ...
    pub async fn shutdown(&self) {
        for entry in &self.ui.sessions {
            let _ = self.backend.close_session(&entry.session_id).await;
        }
    }
}
```
...
### Task 14: Fix session list to show agent name
...
pub fn render_thread_list(
    ...
    let items: Vec<ListItem> = sessions
        ...
        .map(|(i, entry)| {
            ...
            let tid_short = &entry.session_id[..8.min(entry.session_id.len())];
...
In `crates/minos-tui/src/ui/mod.rs`, change the `render_ui` function to pass `&state.sessions` instead of `state.thread_snapshots()`:
...
git commit -m "refactor(tui): session list uses ThreadEntry with agent name"
```
...
Expected: TUI launches, shows "codex" in status bar, session list shows a new entry, chat panel shows agent output
