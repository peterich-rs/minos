...
| Desktop **local-only** start agent (daemon RPC, no cloud start) | HostIngestLiveBatch for unknown `session_id` | **No** — chunks **dropped** (`unknown agent session`) | **Yes (residual)** | Requires cloud `agent_sessions` (+ conversation FK). Product: only project sessions registered on hub; honesty UX is `T-proj-04` |
