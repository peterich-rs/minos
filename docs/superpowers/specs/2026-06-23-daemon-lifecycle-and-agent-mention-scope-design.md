...
**根因:** `room_agent_mention_candidates()` (`crates/minos-tui/src/ui/mod.rs:250`) 从 `self.threads`（全局线程列表，包含所有 conversation 的 agent）而非 `self.conversation_agent_sessions`（当前 conversation 的 session 列表）获取 "existing" 候选。`thread_id_for_agent_short_id()` (`submission.rs:414`) 同样从全局 `self.threads` 解析，保持了一致的错误。
...
`room_agent_mention_candidates()` (`ui/mod.rs:250`) 当前从 `self.threads` 取 "existing" 候选。改为根据当前 nav level 判断：

- **在 conversation 内** (`NavLevel::Conversation` 或更深层): 从 `self.conversation_agent_sessions` 取候选
- **在顶层** (`NavLevel::Agents` 等): 保持从 `self.threads` 取（全局 agent pane 需要看到所有 thread）
...
- `crates/minos-tui/src/backend/daemon.rs` / `embedded.rs` — `list_conversation_agent_sessions` 实现需要返回 state
...
```rust
async fn spawn_daemon_subprocess(discovery_path: &Path) -> Result<()> {
    ...
    // 等待 discovery 文件出现
    wait_for_discovery_file(discovery_path, Duration::from_secs(10)).await
}
```
...
Hard shutdown 时，TUI 通过已有的 local RPC 连接调用一个新方法 `minos_local_shutdown_daemon`（或复用现有机制），daemon 收到后执行 `DaemonHandle::stop()`。

新增 local RPC method:
```rust
#[method(name = "minos_local_shutdown_daemon")]
...
Daemon 的关闭只通过两条路径触发，均走 `minos_local_shutdown_daemon` RPC：
...
| `crates/minos-tui/src/backend/daemon.rs` | `list_conversation_agent_sessions` 返回 state |
...
| `crates/minos-protocol/src/local_rpc.rs` | 新增 `shutdown_daemon` RPC method 定义 |
...
| `crates/minos-tui/src/main.rs` | 新增 `wait_for_discovery_file()` + PID 文件崩溃恢复逻辑 |
...
- 单元测试：顶层 nav level 仍返回全局 thread（不回归）
...
1. 新增 `minos_local_shutdown_daemon` RPC + daemon 端实现
