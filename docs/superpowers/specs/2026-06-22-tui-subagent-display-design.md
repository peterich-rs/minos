...
2. **运行时 / 持久化层**：`AgentManager` 只显式追踪用户启动的顶层线程。Codex pump 对未知 provider session id 会回退用原 id 转发，但没有 `ThreadHandle` / SQLite `sessions` 父行时，daemon `events.session_id` 外键会拒绝持久化；TUI 也会因为没有 `ChatState` 丢弃该 thread 的 ingest。Opencode SSE pump 的 `resolve_session_id` 会主动丢弃所有未注册 sessionID 的事件（`opencode_driver.rs:341`）。
...
- `minos-daemon` / `minos-protocol`：本地 SQLite `sessions` 父子关系持久化 + thread summary 数据形状扩展
...
- Subagent transcript 不能先广播、后补线程父行。必须先注册 `ThreadHandle` 并持久化 `sessions` 行，再释放该 sub_session_id 的 `RawIngest`，否则 daemon event writer 会因为外键失败丢事件。
- TUI 的 `ChatState` 只负责当前 session transcript 投影，不跨线程创建或维护其他 `ChatState`。跨线程父子关系由 `App` / `UiState` 层维护。
...
pub enum UiEventMessage {
    ...
    SubagentSpawned {
        ...
        title: Option<String>,      // optional display label; usually None until SessionOpened/title update
    },
...
Subagent 自己的会话内容使用**现有** `UiEventMessage` 变体，以 `LocalIngestFrame.session_id = sub_session_id` / `RawIngest.session_id = sub_session_id` 为路由 key 流动：
- `SessionOpened { session_id: sub_session_id, agent, ... }`
- `MessageStarted { ... }`, `TextDelta { ... }`, `ToolCallPlaced { ... }` 等
- `SessionClosed { session_id: sub_session_id, ... }`

单个 `UiEventMessage` 的 message/tool 事件本身不携带 session id，不能靠 `ChatState` 内部跨线程路由。TUI 必须在 `App::handle_ingest` 层按 frame session_id 找到或懒创建对应 `ChatState`。
...
pub struct CodexTranslatorState {
    // ... existing ...
    emitted_subagent_ids: HashSet<(String, String)>,  // (collab item id, receiver session id)
}
```

**`translate()` 的 `item/started` 分支：**
- 新增 `"collabAgentToolCall"` arm：解析 `senderThreadId`、`receiverThreadIds`、`model`、`prompt`、`tool`、`status`。若 status 为 `inProgress` 且未发射过，对每个 `receiverThreadId` 发射 `SubagentSpawned`。记录 `(item_id, receiver_session_id)` 到 `emitted_subagent_ids`。
- `title` 不从 collab item 读取。Codex generated schema 中 `agentNickname` / `agentRole` 属于 `Thread`，不是 `CollabAgentToolCallThreadItem`；如果后续 subagent `thread/started` payload 暴露这些字段，则由 `SessionOpened.title` 或 `SessionTitleUpdated` 展示。
...
- 不新增 `SubagentClosed`。终态由 `SubagentStatusUpdated` 表示，transcript 生命周期继续使用现有 `SessionClosed`。
...
Subagent 必须作为普通 thread 被注册，只是带 `parent_session_id`。这样：
- runtime 能为 subagent 建立 `ThreadHandle`
- daemon 能先插入 SQLite `sessions` 父行，再写 `events`
...
**`SessionSummary` / `BackendThreadSnapshot` 新增字段：**
...
`parent_session_id.is_some()` 即表示 subagent，不单独增加 `is_subagent`。

### 3.2 `ManagerEvent` 扩展

不新增单独的 `SubagentSessionAdded`。Subagent 本质上也是 thread，复用 `SessionAdded`，加可选父 id：
...
`LocalManagerEvent::SessionAdded` 同步增加 `parent_session_id`，TUI daemon mode 和 embedded mode 使用同一事件形状。

### 3.3 Codex event pump 路由 (`event_pump_loop`)

**当前问题：** pump 会把未知 provider session id 回退为 logical session id 并广播，但此时没有 `ThreadHandle` / SQLite `sessions` 行 / TUI `ChatState`。结果不是 runtime 立即丢事件，而是后续持久化或 TUI 投影丢事件。

**注册原则：**
- parent collab item 事件可以立即广播，因为 parent thread 已存在
- sub_session_id 的事件必须在 subagent 注册并持久化 parent row 后才能广播
...
```
pump 收到: item/started, params.sessionId = parent_session_id, item.type = collabAgentToolCall
  → pump 直接解析 receiverThreadIds
  → 对每个 receiverThreadId 调用 register_subagent_thread(parent, sub)
  → 在 self.sessions 注册 ThreadHandle { parent_session_id: Some(parent) }
  → 发射 ManagerEvent::SessionAdded { parent_session_id: Some(parent), ... }
  → daemon manager-event bridge 用 parent 的 conversation_id/workspace 插入 subagent sessions 行
  → pump 为 parent_session_id 创建 RawIngest（含 SubagentSpawned）
```
...
```
pump 收到: thread/started, params.sessionId = sub_session_id
  → pump 在 self.sessions 找到 sub_session_id（Phase 1 已注册）
  → translator 正常处理 → SessionOpened { session_id: sub_session_id }
  → pump 为 sub_session_id 创建 RawIngest
```

**竞态处理（推测性注册）：**

Subagent 的 `thread/started` 可能在父线程的 `CollabAgentToolCall` item/started **之前**到达。处理策略：
- pump 遇到未知 `sessionId` 时，不立即广播该 frame，也不写 SQLite
- 将未知 thread 的原始 notification 暂存在 per-instance orphan buffer（key = provider session id）
- 当后续 `CollabAgentToolCall` 到达时，通过 `receiverThreadIds` 匹配 orphan，此时才：
  1. 注册 `ThreadHandle { parent_session_id: Some(parent) }`
  2. 发射 `ManagerEvent::SessionAdded`
  3. 等 daemon session row 可用后，按原顺序释放 orphan buffer 中的 sub_session_id RawIngest
- 若 orphan 线程在 30s 内未被关联到任何 parent，清理并丢弃已缓冲的事件，同时打结构化日志（session_id、workspace、buffered_count）

### 3.4 Opencode SSE pump 自动注册 (`spawn_sse_pump`)

**当前：** `resolve_session_id` 对未知 sessionID 返回 `None` → 事件被丢弃。

**修改后：**
```rust
async fn resolve_session_id_or_register(
    payload: &Value,
    ...
    let pending_task = pending_tasks.lock().await.values()
        ...
        .cloned()?;
    let parent = pending_task.parent_session_id.clone();

    let sub_session_id = format!("opencode-sub-{}", &session_id[..8.min(session_id.len())]);
    session_map.lock().await.insert(sub_session_id.clone(), session_id.clone());
    ...
    manager_tx.send(ManagerEvent::SessionAdded {
    ...
    // Also emit a synthetic parent-session event so TUI renders a SubagentCall card.
    emit_opencode_subagent_spawned(events_tx, &parent, &sub_session_id, &pending_task).await.ok()?;
...
**Pending Task 跟踪：** opencode driver 在 SSE pump 中解析已经归属到 parent session 的 `message.part.updated` 事件：`part.type == "tool"` 且 `part.tool == "task"`。记录 parent opencode session id、parent Minos session id、tool_call_id、prompt、状态。当状态变为 `completed`/`error` 或超过短 TTL 时清除。
...
- 需要结构化日志记录归属决策：parent_session_id、provider_session_id、tool_call_id、workspace、reason

### 3.5 Daemon 持久化

daemon 的 manager-event bridge 处理 `SessionAdded { parent_session_id: Some(parent), ... }` 时：
1. 查询 parent thread 的 `conversation_id`、`workspace_root`
2. 插入 subagent `sessions` 行，`parent_session_id = parent`
3. 不增加 conversation 的 `agent_session_count`（subagent 是顶层 agent run 的子项，不参与 conversation 主计数）
4. 后续该 sub_session_id 的 ingest 才允许写入 `events`

`list_conversation_agent_sessions(conversation_id)` 返回该 conversation 下所有 thread，包括 subagent；TUI 按 `parent_session_id` 渲染树。无需新增 `list_subagents` RPC。
...
pub struct UiState {
    // ... existing ...
    /// sub_session_id → SubagentInfo（模型、prompt、状态）
...
`ThreadEntry` / `SessionSummaryEntry` 增加：

```rust
pub parent_session_id: Option<String>,
```

Sidebar 父子关系以 `SessionSummaryEntry.parent_session_id` 为准，不在 `UiState` 里再维护一份 `subagent_children` / `subagent_parents`，避免三份状态互相漂移。
...
- **正常事件 keyed by `sub_session_id`：** 不在 `ChatState` 内处理。`App::handle_ingest` 按 `LocalIngestFrame.session_id` 找到 sub 的 `ChatState` 后再调用 `apply_ui_events`。

### 4.4 Ingest 路由调整 (`lifecycle.rs`)

`handle_ingest` 当前逻辑：如果 `chat_states` 中没有该 session_id 的条目，则丢弃。

**修改：** 不再直接丢弃未知 thread ingest。对 `LocalIngestFrame`：
- 若 `chat_states` 没有该 `session_id`，用 `frame.agent` 懒创建 `ChatState`
...
- `SessionAdded { parent_session_id: Some(parent), ... }`：在 `ui.sessions` 中添加 `ThreadEntry { parent_session_id: Some(parent), ... }`，并懒创建 `ChatState`
- `SessionClosed` / `SessionStateChanged`：对 subagent 和顶层 thread 使用相同状态更新逻辑
...
- 先取 `parent_session_id.is_none()` 的顶层 session，保持现有排序
- 对每个顶层 session，查 `parent_session_id == top.session_id` 的子条目，按 `last_ts_ms` 或插入顺序展示
- 子条目缩进前缀 `  └ `，显示 agent bin name、短 session id、模型（dimmed）、状态指示器
- 状态指示器：`⠋`（running，可旋转动画）、`✓`（done，绿色）、`✗`（failed，红色）

**导航：** 将树扁平化为一个 `Vec<FlatEntry { session_id, depth, is_subagent }>`。`selected_agent_session` 在 Conversation / AgentDetail 层表示这个扁平列表索引，不再直接等于 `conversation_agent_sessions` 原数组索引。
...
- `UiState::current_session_id()` 在 Conversation / AgentDetail 层读取 selected flat entry
...
现有 `NavLevel::AgentDetail { session_id, ... }` 无需修改。新增：
- 从 sidebar 选中 subagent 条目按 Enter → push `AgentDetail { sub_session_id }`
...
**ChatState 水合：** 导航进入 subagent 时，调用 `read_session_raw_history(sub_session_id, ...)` 拉取历史。

- **Daemon 模式：** 所有 subagent 事件（含 `SubagentSpawned`）作为 `RawIngest` 行持久化到 SQLite，重放时通过翻译器重建 `ChatState`。无需特殊处理。
- **Embedded 模式：** `read_session_raw_history` 返回空（现有 stub 行为，对顶层线程也一样）。subagent transcript 仅通过实时事件积累，不支持重连恢复。这与顶层线程行为一致。
...
- subagent 是 conversation 下某个顶层 session 的子 thread，只需要在线程摘要里带 `parent_session_id`
- 单独 RPC 会制造第二个数据源，TUI 还要同步 `conversation_agent_sessions` 和 `subagent_children`

### 5.2 `SessionSummary` 扩展
...
`SessionSummaryEntry::from_summary` 同步复制该字段。

### 5.3 `BackendThreadSnapshot` / manager event 扩展

`list_sessions()` / manager live events 同步携带 `parent_session_id`，用于 embedded mode 和 direct agent panel 的只读标识。
...
| 文件 | 变更 |
|------|------|
| `src/thread_handle.rs` | `ThreadHandle` 加 `parent_session_id: Option<String>` |
| `src/manager.rs` | `SessionAdded` 携带 `parent_session_id`；`event_pump_loop` 轻量解析 Codex collab item、注册 subagent、缓冲/释放 orphan events |
| `src/manager_event.rs` | `SessionAdded` 加 `parent_session_id: Option<String>` |
| `src/opencode_driver.rs` | `resolve_session_id` → `resolve_session_id_or_register`；driver 层 pending Task 跟踪 + synthetic parent RawIngest |
| `src/store_facing.rs` | `ThreadSnapshot` 加 `parent_session_id` 字段 |

### `minos-daemon`
| 文件 | 变更 |
|------|------|
| `migrations/0001_initial.sql` | `sessions` 表新增 `parent_session_id`，更新索引/fixtures |
| `src/store/mod.rs` | session row / list query / insert API 读写 `parent_session_id`；subagent 不增加 conversation agent_session_count |
| `src/store/event_writer.rs` | 保持 events FK 等待逻辑，新增测试覆盖 subagent session row 先写入 |
| `src/agent.rs` | manager event bridge 按 parent thread 的 conversation/workspace 插入 subagent session row |
| `src/local_rpc.rs` | `LocalManagerEvent::SessionAdded` 转发 `parent_session_id` |

### `minos-tui`
| 文件 | 变更 |
|------|------|
| `src/translation/chat_item.rs` | 新增 `SubagentCall` 变体；`message_id()`/`set_streaming()` 覆盖新变体 |
| `src/translation/chat_state.rs` | `apply_ui_event` 加 `SubagentSpawned`/`StatusUpdated` 分支，只更新当前 session transcript |
| `src/translation/tool_summary.rs` | 可选：为 SubagentCall 提供 prompt 摘要辅助函数 |
| `src/ui/mod.rs` | `UiState` 加 `subagent_info`；`ThreadEntry` / `SessionSummaryEntry` 加 `parent_session_id`；current thread selection 走 flat helper |
| `src/ui/conversation_detail.rs` | `AgentSessionListRenderable` 支持树状渲染 + 扁平化导航 |
| `src/ui/chat.rs` | `build_item_lines` 加 `SubagentCall` 渲染分支 |
| `src/app/lifecycle.rs` | `handle_ingest` 对未知 thread 懒创建 ChatState；处理 `parent_session_id` manager event；subagent input 只读 |
| `src/app/event_mapping.rs` | Conversation sidebar Up/Down 使用扁平化索引 |
| `src/app/event_loop.rs` | `OpenAgentSession` 支持 subagent session_id 历史水合 |
| `src/backend/mod.rs` | `BackendThreadSnapshot` / `SessionSummaryEntry` 加 `parent_session_id` |
| `src/backend/embedded.rs` | 复制 manager/list_sessions 的 `parent_session_id` |
| `src/backend/daemon.rs` | 从 RPC `SessionSummary` 复制 `parent_session_id` |

### `minos-protocol`
| 文件 | 变更 |
|------|------|
| `src/messages.rs` | `SessionSummary` 加 `parent_session_id` |
| `src/local_rpc.rs` | `LocalManagerEvent::SessionAdded` 加 `parent_session_id` |
...
1. `sessions.parent_session_id` schema 更新（直接更新 canonical migration）
2. `SessionSummary` / `LocalManagerEvent::SessionAdded` / `BackendThreadSnapshot` 加 `parent_session_id`
...
1. `ThreadHandle` / `ManagerEvent::SessionAdded` 携带 `parent_session_id`
2. Codex event pump 直接轻量解析 collab item，注册 subagent session，缓冲/释放 orphan sub events
...
4. 单元测试：事件→ChatItem 翻译；sub_session_id ingest 路由到 sub ChatState
...
**协议层：**
- Codex 翻译器：`collabAgentToolCall` item/started JSON → `SubagentSpawned`（验证 model/prompt/session_ids 提取）
...
**持久化层：**
- `sessions.parent_session_id` 插入/查询正确
- `list_conversation_agent_sessions` 返回顶层 session + subagent session
- subagent session insert 不增加 conversation `agent_session_count`
- subagent event 写入前已有 thread parent row，避免 `events.session_id` 外键失败

**运行时层：**
- Codex parent-first：collab item 到达后注册 subagent，再广播 sub session event
- Codex child-first：未知 sub session event 先缓冲；collab item 到达后注册并按顺序释放
...
- sub_session_id 的 ingest 帧 → `handle_ingest` 懒创建并路由到 sub ChatState
...
- Codex child-first fixture：sub session event 早于 collab item 时不丢 transcript
...
| Codex 事件获取 | 注册 sub thread 后再释放其 RawIngest | 避免 SQLite `events.session_id` FK 失败和 TUI 丢 ChatState |
...
| 竞态处理 | unknown sub session events 先缓冲 | 子事件早于 parent collab item 时不丢 transcript，也不提前写无父 row |
| Backend 数据源 | 扩展 `SessionSummary.parent_session_id` | 复用现有 `list_conversation_agent_sessions`，不新增 `list_subagents` 第二数据源 |
