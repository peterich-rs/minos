...
Conversation 元数据（`conversations` 表，见 `0001_initial`）：`priority`、`progress`（默认 `todo`）、`branch` / `worktree_path`（**创建时**从 project workspace 做 git 快照）。Agent **roster** 在 `conversation_agent_members`（runtime 名：`codex`/`claude`/…）；`create_conversation` 接受可选 `priority` 与 `agents[]` 写入 roster。`LocalConversationSummary.participating_agents` **来自 roster，不是 sessions**。`start_agent_in_conversation` 要求目标 agent 已是成员，否则 protocol error。`update_conversation` 可改 title / priority / progress；列表同时聚合 `running_count` 与 `needs_attention_count`（suspended sessions）。首次 `start_agent_in_conversation` 时若 progress 仍为 `todo` 则升为 `in_progress`。
