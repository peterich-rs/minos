...
import { daemonApi, isTauriRuntime } from "@/lib/daemon";
...
function CreateAgentDialog({
  ...
  clis: { agent: string; installed: boolean }[];
  onClose: () => void;
  onCreated: () => void | Promise<void>;
}) {
  const installed = useMemo(
    () => new Set(clis.filter((c) => c.installed).map((c) => c.agent)),
    [clis],
  );
  const defaultRuntime =
    RUNTIMES.find((r) => installed.has(r)) ?? ("codex" as AgentRuntime);
  ...
  const loadModels = useCallback(async (rt: string) => {
    ...
    try {
      ...
      setModel(def?.id ?? "");
      if (def?.default_reasoning_effort) {
        setEffort(def.default_reasoning_effort);
      } else if (def?.supported_reasoning_efforts?.length) {
        setEffort(def.supported_reasoning_efforts[0]!);
      } else {
        setEffort("");
      }
      setErr(null);
  ...
  const effortOptions = selectedModelMeta?.supported_reasoning_efforts?.length
    ? selectedModelMeta.supported_reasoning_efforts
    : ["low", "medium", "high"];
          ...
          <Field label="Runtime">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {RUNTIMES.map((r) => {
                ...
                const isOn = installed.has(r);
                const selected = runtime === r;
                return (
                  <button
                    key={r}
                    type="button"
          ...
          >
            <select
              ...
              onChange={(e) => {
                ...
                if (m?.default_reasoning_effort) {
                  setEffort(m.default_reasoning_effort);
                } else if (m?.supported_reasoning_efforts?.[0]) {
                  setEffort(m.supported_reasoning_efforts[0]);
                }
              }}
            ...
            <p className="mt-1.5 text-[11px] text-ink-muted">
              {loadingModels
                ? `Discovering models for ${agentMeta[runtime]?.label ?? runtime}…`
                : modelsSource
