...
void main() {
  testWidgets(
    'Empty submit shows validation errors and does not call onSubmit',
    (tester) async {
      var submits = 0;
  ...
  testWidgets('inFlight=true disables the submit button and shows a spinner', (
    ...
    final btn = tester.widget<ShadButton>(find.byType(ShadButton).first);
    expect(btn.enabled, isFalse);
    expect(find.byType(CircularProgressIndicator), findsOneWidget);
  });

  testWidgets(
    'Mode toggle reveals the confirm-password field in register mode',
    (tester) async {
  ...
  testWidgets('Register-mode confirm mismatch surfaces an error', (
    tester,
  ) async {
    await tester.pumpWidget(
      _harness(
