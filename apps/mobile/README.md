...
Dart ↔ Rust bindings under `lib/src/rust/` are generated by
`flutter_rust_bridge_codegen` **2.12.0** (lockstep with
`crates/minos-ffi-frb` Cargo pin and this package's `flutter_rust_bridge: 2.12.0`).

After changing `crates/minos-ffi-frb/src/api/**` mirrors or exposed APIs:
...
First-time install of the matching codegen tool:

```sh
cargo xtask bootstrap
```

Do not hand-edit `lib/src/rust/**` or `crates/minos-ffi-frb/src/frb_generated.rs`
