...
class _AgentEditorSheetState extends ConsumerState<AgentEditorSheet> {
  ...
  void initState() {
    super.initState();
    _initialDraft = widget.profile == null
        ? _seedDraft(widget.hosts)
        : AgentProfileDraft.fromProfile(widget.profile!);
    _nameController = TextEditingController(text: _initialDraft.name);
  ...
  String _hostSelectionLabel(AgentProfileDraft draft) {
    final explicit = draft.hostDisplayName?.trim();
    if (explicit != null && explicit.isNotEmpty) return explicit;
    final fromList = _hostLabelForId(widget.hosts, draft.hostDeviceId);
  ...
  Future<void> _pickHost() async {
    ...
    final selected = await _showPickerSheet<String?>(
      title: '选择运行设备',
      options: hostOptions,
      currentValue: draft.hostDeviceId,
    );
    if (!mounted) return;
  ...
  Future<void> _pickRuntime(List<AgentName> runtimeOptions) async {
    ...
    final selected = await _showPickerSheet<AgentName>(
      ...
      currentValue: draft.runtimeAgent,
    );
    if (selected == null || !mounted) return;
    ref
        .read(agentEditorDraftControllerProvider(_initialDraft).notifier)
        .setRuntime(selected, defaultModel: _defaultModel(selected));
  }
  ...
  Future<void> _saveProfile() async {
    ...
    logFlutterInfo(
      'agent_editor',
      'save profile requested isEditing=${widget.profile != null} runtime=${draft.runtimeAgent} model=${draft.model} hostDeviceId=${draft.hostDeviceId ?? '<active>'}',
    );
    try {
      if (widget.profile == null) {
        ...
        logFlutterInfo(
          'agent_editor',
          'profile created profileId=${created.id} runtime=${draft.runtimeAgent}',
        );
      } else {
        await controller.updateProfile(widget.profile!, draft);
        logFlutterInfo(
          'agent_editor',
          'profile updated profileId=${widget.profile!.id} runtime=${draft.runtimeAgent}',
        );
      }
      ...
      logFlutterError(
        'agent_editor',
        'save profile failed isEditing=${widget.profile != null}',
        error: error,
      ...
      showLoggedErrorToast(
        context,
        target: 'agent_editor',
        title: widget.profile == null ? '创建 Agent 失败' : '保存 Agent 失败',
        error: error,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    final editorState = ref.watch(
      agentEditorDraftControllerProvider(_initialDraft),
    );
                    ...
                    _EditorSection(
                      ...
                      child: Column(
                        children: <Widget>[
                          _EditorPickerTile(
                            label: '运行设备',
                            value: _hostSelectionLabel(draft),
                            onTap: _pickHost,
                          ),
                          const Divider(height: 1),
                          _EditorPickerTile(
                            label: 'Runtime',
                            value: _runtimeLabel(draft.runtimeAgent),
                    ...
                    _EditorSection(
                      ...
                      child: Padding(
                        ...
                        child:
                            ...
                            >(
                              groupValue: draft.reasoningEffort,
                              ...
                              onValueChanged: (value) {
                                ...
                                ref
                                    .read(
                                      agentEditorDraftControllerProvider(
                                        _initialDraft,
                                      ).notifier,
                                    )
                                    .setReasoning(value);
                    ...
                    _EditorSection(
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          if (editorState.showAdvanced) ...<Widget>[
                            const Divider(height: 1),
                            Padding(
                            ...
                            for (
                              var i = 0;
                              i < draft.environmentVariables.length;
                              i++
                            )
                              Padding(
                                padding: const EdgeInsets.fromLTRB(
                                  ...
                                  child: Row(
                                    children: <Widget>[
                                      Expanded(
                                        child: TextFormField(
                                          initialValue:
                                              draft.environmentVariables[i].key,
