...
import 'package:minos/ui/features/pairing/views/permission_denied_page.dart';
import 'package:minos/ui/features/pairing/widgets/qr_scanner_view.dart';
import 'package:permission_handler/permission_handler.dart';
import 'package:shadcn_ui/shadcn_ui.dart';

final _pairingCandidateProvider = NotifierProvider.autoDispose
    .family<_PairingCandidateController, _PairingCandidate?, String>(
      _PairingCandidateController.new,
    );

class _PairingCandidateController extends Notifier<_PairingCandidate?> {
  _PairingCandidateController(String _);

  @override
  _PairingCandidate? build() => null;

  void select(_PairingCandidate candidate) {
...
class _PairingPageState extends ConsumerState<PairingPage> {
  ...
  late final String _pageId = 'pairing-page-${identityHashCode(this)}';
  ...
  Widget build(BuildContext context) {
    ...
    final candidate = ref.watch(_pairingCandidateProvider(_pageId));
    if (candidate != null) {
  ...
  Widget _buildForStatus(PermissionStatus status) {
    ...
    if (!_requestedOnce) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          unawaited(ref.read(cameraPermissionProvider.notifier).request());
        }
      });
    }
  ...
  bool _handleDetected(String raw) {
    ...
    ref.read(_pairingCandidateProvider(_pageId).notifier).select(candidate);
    return true;
  }
