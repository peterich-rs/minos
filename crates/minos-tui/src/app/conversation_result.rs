...
impl App {
    ...
    async fn record_agent_conversation_result(
        ...
        let Some(chat) = self.ui.session_panel.chat_states.get(session_id) else {
            return;
        };
        ...
        if self
            .append_agent_result_to_conversation(session_id, &message_key, &text)
            .await
        {
            self.state
                .recorded_agent_results
                .insert(session_id.to_owned(), message_key);
        }
    }
    ...
    async fn append_agent_result_to_conversation(
        ...
        let Some(conversation_id) = self.conversation_id_for_thread(session_id) else {
            return false;
        };
        let message_id =
            conversation_agent_result_message_id(&conversation_id, session_id, message_key);
        let delegation = self
            .state
            .teamwork_store
    ...
    async fn complete_and_deliver_delegation_result(
        ...
        {
            Ok(Some(_)) => {
                self.deliver_delegation_result_to_source(delegation, session_id, visible_body)
                    .await;
            }
            Ok(None) => {}
    ...
    fn conversation_id_for_thread(&self, session_id: &str) -> Option<String> {
        self.state
...
fn delegation_result_visible_message(
    ...
    let source_session_id = delegation.source_session_id.as_deref()?;
    Some(format!(
        "@{}#{} {}",
        source_agent.bin_name(),
        short_session_id(source_session_id),
        text.trim()
    ))
}
