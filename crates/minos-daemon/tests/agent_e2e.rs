...
async fn register_formal_host(
    ...
    { let _acct = store::accounts::create(&relay.pool, &format!("fixture-{}@localhost", mobile_id)).await.unwrap(); store::test_support::insert_test_client(&relay.pool, mobile_id, DeviceRole::MobileClient, &_acct.account_id, "Test iPhone", 0,).await; };
