...
pub async fn connect_sqlite_with_options(
    ...
    ensure_polymorphic_message_mentions_sqlite(&pool).await?;

    Ok(pool)
}

/// Rebuild `chat_message_mentions` to polymorphic `(target_kind, target_id)` when
/// an existing volume still has pre-bot `mentioned_account_id`. No-op when the
/// table is already polymorphic (fresh 0001 or previously migrated).
///
/// Lives in Rust because SQLite cannot branch on column existence in pure SQL
/// without prepare-time failures on the missing column names.
pub async fn ensure_polymorphic_message_mentions_sqlite(
    pool: &SqlitePool,
) -> Result<(), BackendError> {
    let cols: Vec<String> = sqlx::query_scalar(
        "SELECT name FROM pragma_table_info('chat_message_mentions')",
    )
    .fetch_all(pool)
    .await
    .map_err(|e| BackendError::StoreMigrate {
        message: format!("list chat_message_mentions columns: {e}"),
    })?;

    let has_legacy = cols.iter().any(|c| c == "mentioned_account_id");
    let has_polymorphic = cols.iter().any(|c| c == "target_kind");
    if has_polymorphic || !has_legacy {
        // Already target shape, or table missing/empty of known shapes — leave alone.
        if has_polymorphic {
            // Ensure index exists for volumes that got polymorphic 0001 without 0002 index.
            sqlx::query(
    ...
    let mut tx = pool.begin().await.map_err(|e| BackendError::StoreMigrate {
        message: format!("begin polymorphic mentions rebuild: {e}"),
    })?;

    sqlx::query(
        "CREATE TABLE chat_message_mentions__new (
    ...
    .await
    .map_err(|e| BackendError::StoreMigrate {
        message: format!("create polymorphic mentions table: {e}"),
    })?;
    ...
    .await
    .map_err(|e| BackendError::StoreMigrate {
        message: format!("copy legacy account mentions: {e}"),
    })?;

    sqlx::query("DROP INDEX IF EXISTS idx_chat_message_mentions_account")
        .execute(&mut *tx)
        .await
        .map_err(|e| BackendError::StoreMigrate {
            message: format!("drop legacy mentions index: {e}"),
        })?;
    sqlx::query("DROP TABLE chat_message_mentions")
        .execute(&mut *tx)
        .await
        .map_err(|e| BackendError::StoreMigrate {
            message: format!("drop legacy mentions table: {e}"),
        })?;
    sqlx::query("ALTER TABLE chat_message_mentions__new RENAME TO chat_message_mentions")
        .execute(&mut *tx)
        .await
        .map_err(|e| BackendError::StoreMigrate {
            message: format!("rename polymorphic mentions table: {e}"),
        })?;
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_chat_message_mentions_target
         ON chat_message_mentions(target_kind, target_id, message_id)",
    ...
    tx.commit().await.map_err(|e| BackendError::StoreMigrate {
        message: format!("commit polymorphic mentions rebuild: {e}"),
    })?;
    Ok(())
}
...
pub mod test_support {
    ...
    pub async fn memory_pool() -> SqlitePool {
        ...
        super::ensure_polymorphic_message_mentions_sqlite(&pool)
            .await
            .unwrap();
        pool
    }
...
mod tests {
    ...
    /// Simulate a volume that still has pre-bot `mentioned_account_id` and verify
    /// the post-migrate rebuild produces polymorphic rows + is idempotent.
    #[tokio::test]
    async fn ensure_polymorphic_rebuilds_legacy_mentioned_account_id() {
        use super::ensure_polymorphic_message_mentions_sqlite;
        ...
        // Minimal legacy graph: accounts → conversations → chat_messages → mentions.
        sqlx::query(
        ...
        sqlx::query("INSERT INTO accounts (account_id, email) VALUES ('acct-bob', 'bob@ex.com')")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("INSERT INTO conversations (conversation_id) VALUES ('c1')")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query(
            "INSERT INTO chat_messages (message_id, conversation_id) VALUES ('m1', 'c1')",
        )
        .execute(&pool)
        .await
        .unwrap();
        ...
        ensure_polymorphic_message_mentions_sqlite(&pool)
            .await
            .expect("legacy rebuild");

        let cols: Vec<String> =
            sqlx::query_scalar("SELECT name FROM pragma_table_info('chat_message_mentions')")
                .fetch_all(&pool)
                .await
                .unwrap();
        assert!(cols.iter().any(|c| c == "target_kind"));
        assert!(cols.iter().any(|c| c == "target_id"));
        assert!(!cols.iter().any(|c| c == "mentioned_account_id"));

        let row: (String, String) = sqlx::query_as(
            "SELECT target_kind, target_id FROM chat_message_mentions WHERE message_id = 'm1'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(row.0, "account");
        assert_eq!(row.1, "acct-bob");

        // Idempotent on already-polymorphic shape.
        ensure_polymorphic_message_mentions_sqlite(&pool)
            .await
            .expect("second ensure is no-op");
        let n: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM chat_message_mentions WHERE message_id = 'm1'")
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(n, 1);
