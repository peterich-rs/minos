    ...
    match store.as_store_pool() {
        StorePoolRef::Sqlite(pool) => {
            sqlx::query_as::<_, AgentSessionRow>(
                "SELECT session_id, conversation_id, project_id, host_device_id AS host_device_id, agent_id, status, started_at_ms, ended_at_ms
                   FROM agent_sessions
                  WHERE session_id = ?",
            )
            .bind(session_id)
        ...
        StorePoolRef::Postgres(pool) => {
            sqlx::query_as::<_, AgentSessionRow>(
                "SELECT session_id, conversation_id, project_id, host_device_id AS host_device_id, agent_id, status::text, started_at_ms, ended_at_ms
                   FROM agent_sessions
                  WHERE session_id = $1",
            )
            .bind(session_id)
...
pub async fn get_for_account(
    ...
    match store.as_store_pool() {
        StorePoolRef::Sqlite(pool) => {
            sqlx::query_as::<_, AgentSessionRow>(
                "SELECT s.session_id, s.conversation_id, s.project_id, s.host_device_id AS host_device_id, s.agent_id, s.status, s.started_at_ms, s.ended_at_ms
        ...
        StorePoolRef::Postgres(pool) => {
            sqlx::query_as::<_, AgentSessionRow>(
                "SELECT s.session_id, s.conversation_id, s.project_id, s.host_device_id AS host_device_id, s.agent_id, s.status::text, s.started_at_ms, s.ended_at_ms
                   FROM agent_sessions s
                   JOIN conversation_members cm
                     ON cm.conversation_id = s.conversation_id
