...
use minos_protocol::{
    ...
    ConversationMembersResponse, ConversationParticipantsResponse,
    ConversationReadResponse, ConversationResponse,
...
impl MobileHttpClient {
    ...
    pub async fn toggle_reaction(
        ...
        let trace_id = start_http_trace(
            ...
            &path,
            Some(conversation_id.into()),
            Some(format!("react={emoji}")),
        );
        let req = minos_protocol::ToggleReactionRequest {
        ...
        let status = resp.status();
        if status.is_success() {
            let body = decode_success_json(resp, "ToggleReactionResponse").await?;
            request_trace::finish_success(
                trace_id,
                Some(status.as_u16()),
                Some("reaction toggled".into()),
                Some(conversation_id.into()),
            );
            Ok(body)
        } else {
    ...
    /// `POST /v1/agent-sessions/respond-opencode-question` — answer an
    /// opencode question request for an active agent session.
    pub async fn respond_opencode_question(
        ...
        let path = "/v1/agent-sessions/respond-opencode-question";
        let url = format!("{}{path}", self.base);
        let trace_id = start_http_trace(
            Method::POST.as_str(),
            path,
            Some(session_id.to_owned()),
            Some(format!("question_id={question_id}")),
        );
        let body = RespondOpencodeQuestionRequest {
            session_id: session_id.into(),
        ...
        if status.is_success() {
            request_trace::finish_success(
                trace_id,
                Some(status.as_u16()),
                Some("opencode question answered".into()),
                Some(session_id.to_owned()),
            );
            Ok(())
        } else {
