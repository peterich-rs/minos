...
sync-local-env:
    ...
    set -euo pipefail
    if [[ ! -f .env.local ]]; then
        echo "error: .env.local missing — copy from .env.example and fill secrets"
        exit 1
    fi
    ...
    {
        echo "# Generated by just sync-local-env from repo-root .env.local"
        echo "VITE_MINOS_BACKEND_URL=$${VITE_MINOS_BACKEND_URL:-http://127.0.0.1:8787}"
        echo "VITE_SUPABASE_URL=$${VITE_SUPABASE_URL:-$${SUPABASE_URL:-}}"
        echo "VITE_SUPABASE_ANON_KEY=$${VITE_SUPABASE_ANON_KEY:-$${SUPABASE_ANON_KEY:-}}"
